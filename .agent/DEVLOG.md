...
## 59. Native Android Share Clipper Integration (June 2026)

- **JNI Bridge Extension & Endpoints:**
  - **Task:** Implement Android JNI hooks `check_url_history` and `clipUrl` in `crates/android-bridge/src/lib.rs` and `com.bubi.nodanotes.RustCore`.
  - **Solution:** 
    - `check_url_history(url: String)`: Queries SQLite active notes table to check if the target URL has already been clipped to Noda using a fast `body LIKE ?1` check. Returns `{ "exists": true, "note_id": "ULID" }` or `{ "exists": false }`.
    - `clipUrl(inputJson: String)`: Parses ClipperPayload. If the page content is not supplied (Full Page Share), it fetches HTML via `reqwest`, parses it through `readability` crate's `extract` engine to extract cleaned article text/body, and runs a custom regex-based `html_to_markdown` conversion. Appending is boundary-aware; it locates `<!-- id: nodanotes-webclipper-boundary -->` in the target file and inserts paragraphs above it, padded with newline spaces and a dynamic date signature (`*Appended via Android on YYYY-MM-DD:*`). For new notes, it sanitizes the webpage title for destination file path mapping, compiles a formatted metadata footer block, writes to disk, calculates the XXH3 hash of the markdown, upserts to the database cache, and updates `sync_file_states` (setting `is_dirty = 1`) to mark the file as ready for background WebDAV synchronization.
- **Cross-Compilation OpenSSL Resolution:**
  - **Problem:** Compiling the JNI bridge with `reqwest` pulled in `native-tls` and `openssl-sys`, which failed during Android cross-compilation because of missing platform-specific OpenSSL headers/libraries.
  - **Solution:** Added `openssl = { version = "0.10", features = ["vendored"] }` to the dependencies in `crates/android-bridge/Cargo.toml` and configured `reqwest` features to explicitly use `rustls-tls` where possible. The vendored feature triggers compiler compilation of OpenSSL directly from sources inside cargo, resolving the Android target build failure.
